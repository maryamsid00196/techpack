...
def save_uploaded_image(uploaded_file, prefix):
    """Save uploaded image safely in /uploads and return path."""
...
if excel_file:
    st.session_state.excel_data = read_excel(excel_file)
    st.success(f"Excel uploaded. Rows detected: {len(st.session_state.excel_data)}")

# --- STEP 1: UPLOAD LOGO ---
st.subheader("Step 1: Upload Logo Image")
logo_file = st.file_uploader("Upload Logo", type=["png", "jpg", "jpeg"])
if logo_file:
    st.session_state.logo_path = save_uploaded_image(logo_file, "logo")
    st.success("Logo uploaded.")

# --- STEP 2: SIZE INPUT ---
st.subheader("Step 2: Logo Size (for report text)")
...
if cap_file:
    cap_path = save_uploaded_image(cap_file, "cap")
...
if st.button("▶️ Run Process"):
    if not st.session_state.logo_path or not st.session_state.current_cap_filename:
        st.error("Please upload both logo and cap image before running.")
    else:
        ...
        if json_data and json_data.get("objects"):
            ...
            if len(points) >= 4:
                ...
                st.warning("Please draw at least 4 points on the canvas.")
